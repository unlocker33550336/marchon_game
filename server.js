// server.js (パタパタ開示データ転送・1000%完全版)
...
io.on('connection', (socket) => {
    ...
    socket.on('submit_turn', (data) => {
        ...
        if (rooms.choices.p1 && rooms.choices.p2) {
            ...
            // 🌟 画面側がパタパタ開示できるように、お互いの配置生データを乗せる！
...
function calculateOfficialLogic(c1, c2, state, evReserved, lastEv) {
    ...
    ['p1', 'p2'].forEach(pKey => {
        if(evReserved[pKey]) {
            ...
            else if(maxType === 'stone') { oppData.progress = Math.max(0, oppData.progress - maxCount); evText += `★P${pKey==='p1'?1:2}：【避けられぬ現実】相手を ${maxCount}% 後退させた！\\n`; }
    ...
    let resLog = `========================================\\n` +
                 ...
                 (evText ? evText : "") +
                 `👤 P1 -> [自分:${c1.self}] [相手:${c1.target}]\\n` +
                 `👥 P2 -> [自分:${c2.self}] [相手:${c2.target}]\\n` +
                 `========================================`;
